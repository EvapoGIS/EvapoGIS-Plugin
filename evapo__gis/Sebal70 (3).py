...
W = [ESUN[i] / sum(ESUN) for i in range(len(ESUN))]
W1 = W[0]
W2 = W[1]
W3 = W[2]
W4 = W[3]
...
xy_Pcold = input('Place coordinates (east,north): ').strip('()')
print(xy_Pcold)
...
grass.mapcalc('L01m = if($L < 0, 2 * log((1 + (1 - 16 * (0.1 / $L))^0.5) / 2), if($L > 0, -5 * (0.1 / $L), 0))',
              ...
              quiet='true')
print("Done!")
print("Calculating corrected friction velocity map (u_astmap) - m/s...")
